...
(function() {
  ...
  ContentEdit = {
    ...
    DEFAULT_MAX_ELEMENT_WIDTH: 10000,
...
(function() {
  ...
  ContentTools = {
    ...
    DEFAULT_TOOLS: [['bold', 'italic', 'link', 'align-left', 'align-center', 'align-right'], ['heading', 'subheading', 'paragraph', 'unordered-list', 'ordered-list', 'table', 'indent', 'unindent', 'line-break'], ['insertImage', 'video', 'preformatted'], ['undo', 'redo', 'remove']],
  ...
  ContentTools.ImageInsertionDialog = (function(_super) {
    __extends(ImageInsertionDialog, _super);

    function ImageInsertionDialog(src) {
      if (src == null) {
        src = '';
      }
      ImageInsertionDialog.__super__.constructor.call(this);
      this.src = src;
    }

    ImageInsertionDialog.prototype.mount = function() {
      ImageInsertionDialog.__super__.mount.call(this);
      this._domInput = document.createElement('input');
      this._domInput.setAttribute('class', 'ct-anchored-dialog__input-for-single-button');
      this._domInput.setAttribute('name', 'src');
      this._domInput.setAttribute('placeholder', ContentEdit._('Enter image url') + '...');
      this._domInput.setAttribute('type', 'text');
      this._domInput.setAttribute('value', this.src);
      this._domElement.appendChild(this._domInput);
      this._domButton = this.constructor.createDiv(['ct-anchored-dialog__button']);
      this._domElement.appendChild(this._domButton);
      return this._addDOMEventListeners();
    ...
    ImageInsertionDialog.prototype.save = function() {
      var detail, srcValue;
      if (!this.isMounted()) {
        this.dispatchEvent(this.createEvent('save'));
        return;
      }
      srcValue = this._domInput.value.trim();
      if (!srcValue) {
        this.dispatchEvent(this.createEvent('save'));
        return;
      }
    ...
    ImageInsertionDialog.prototype.show = function() {
      ImageInsertionDialog.__super__.show.call(this);
      this._domInput.focus();
      if (this._href) {
        return this._domInput.select();
      }
    ...
    ImageInsertionDialog.prototype.unmount = function() {
      if (this.isMounted()) {
        this._domInput.blur();
      }
      ImageInsertionDialog.__super__.unmount.call(this);
      this._domButton = null;
      return this._domInput = null;
    };

    ImageInsertionDialog.prototype._addDOMEventListeners = function() {
      this._domInput.addEventListener('keypress', (function(_this) {
        return function(ev) {
          if (ev.keyCode === 13) {
            return _this.save();
  ...
  ContentTools.Tools.ImageInsertion = (function(_super) {
    var thisFunction;
    ...
    ContentTools.ToolShelf.stow(ImageInsertion, 'insertImage');

    ImageInsertion.label = 'InsertImage';

    ImageInsertion.icon = 'image';

    thisFunction = ImageInsertion;

    ImageInsertion.getImageSizeFromUrl = function(url, callback) {
      var img, imgSize;
      img = new Image();
      imgSize = [0, 0];
      img.src = url;
      return img.addEventListener('load', (function(_this) {
        return function() {
    ...
    ImageInsertion.canApply = function(element, selection) {
      if (element.isFixed()) {
        if (element.type() !== 'ImageFixture') {
          return false;
        }
      ...
      if (selection !== null && selection.isCollapsed !== null && !selection.isCollapsed()) {
        return false;
      }
      return true;
    };

    ImageInsertion.apply = function(element, selection, callback) {
      var allowScrolling, app, dialog, modal, rect, scrollX, scrollY, toolDetail, transparent, _ref;
      toolDetail = {
        'tool': this,
        'element': element,
        'selection': selection
      ...
      modal.addEventListener('click', function() {
        ...
        callback(applied);
        if (applied) {
          return ContentTools.Tools.ImageInsertion.dispatchEditorEvent('tool-applied', toolDetail);
        }
      });
      dialog = new ContentTools.ImageInsertionDialog();
      rect = element.domElement().getBoundingClientRect();
      ...
      dialog.addEventListener('save', (function(_this) {
        return function(ev) {
          var detail, imageSize, imageURL, index, loadImageSizeCallback, node, _ref1;
          detail = ev.detail();
          imageURL = detail.imageURL;
          _ref1 = _this._insertAt(element), node = _ref1[0], index = _ref1[1];
          thisFunction.dispatchEditorEventFunction = _this.dispatchEditorEvent;
          loadImageSizeCallback = function(imgSize) {
            var containerWidth, image, imageAttrs, radio;
            imageAttrs = {};
            imageAttrs.width = imgSize[0];
            imageAttrs.height = imgSize[1];
            imageAttrs.src = imageURL;
            containerWidth = rect.width;
            if (imageAttrs.width > containerWidth) {
              radio = containerWidth / imageAttrs.width;
              imageAttrs.width = containerWidth;
              imageAttrs.height = imageAttrs.height * radio;
            }
            if (element.type() === 'ImageFixture') {
              element.src(imageURL);
            } else {
            ...
            dialog.hide();
            callback(true);
            return thisFunction.dispatchEditorEventFunction('tool-applied', toolDetail);
          };
          return imageSize = _this.getImageSizeFromUrl(detail.imageURL, loadImageSizeCallback);
        };
      })(this));
      app.attach(modal);
      app.attach(dialog);
